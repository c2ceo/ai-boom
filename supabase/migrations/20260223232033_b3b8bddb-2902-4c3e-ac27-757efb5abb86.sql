...
BEGIN
  ...
  SELECT (sp).id, (sp).user_id, (sp).image_url, (sp).video_url, (sp).caption, (sp).category,
         (sp).ai_tool, (sp).tags, (sp).likes_count, (sp).comments_count, (sp).views_count,
         (sp).is_verified_ai, (sp).is_family_friendly, (sp).status, (sp).voting_expires_at,
         (sp).created_at, (sp).updated_at
  FROM (SELECT (sp).* FROM scored_posts sp ORDER BY sp.score DESC LIMIT p_limit) sub
  CROSS JOIN LATERAL (SELECT sub.*) AS wrap;
END;
$$;
